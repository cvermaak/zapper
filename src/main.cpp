...
#if defined(NDEBUG)
# error "zapper cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x49d2fe879a7444e9435101b75815a6e64e9deb9173ad62168eb6c57615ea0e2a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // zapper: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "zapper Signed Message:\n";
...
{
    // zapper: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // zapper: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // zapper: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // zapper: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // zapper: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // zapper: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf1000925b4079000c672c1efcf9cae2b7ad0990beaf6a0bcdc7c2776891046b7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On August 22, 2016, Herman Mashaba (DA) sworn in as Mayor of Johannesburg";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045ab18dcf74bf40d00986d46ee7045e751d99d9153ab6de254e0161173d1ba3b6adc0a036f83e15cb9230f5b0f3cf9b8ad49a0c690096d33401967f4bf5584a3d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515676773;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086374463;

        if (fTestNet)
        {
            block.nTime    = 1515676655;
            block.nNonce   = 387843208;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0b70efdef2aa99bb1fd15b38436bbc33b20abdaaee820477f106269e17592c78"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc7, 0xb0, 0xd8 }; // zapper: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("zapperMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("zapperMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static zapperMiner(CWallet *pwallet)
{
    printf("zapperMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zapper-miner");
    ...
    try { loop {
        ...
        printf("Running zapperMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&zapperMiner, pwallet));
}
